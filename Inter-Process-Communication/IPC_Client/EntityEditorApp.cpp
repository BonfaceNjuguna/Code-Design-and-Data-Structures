...
void EntityEditorApp::Update(float deltaTime) {
	...
	SharedData myData = { ENTITY_COUNT };

	HANDLE h = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, sizeof(SharedData), L"MySharedMemory");

	if (h == FALSE)
	{
		std::cout << "CreateFileMapping Failed and Error No: " << GetLastError() << std::endl;
	}

	SharedData* data = (SharedData*)MapViewOfFile(h, FILE_MAP_ALL_ACCESS, 0, 0,
		sizeof(SharedData));

	if (data == NULL)
	{
		std::cout << "MapViewOfFile failed and Error No: " << GetLastError() << std::endl;
	}
